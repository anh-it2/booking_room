...
export const CellAction: React.FC<CellActionProps> = ({ data, setLocations }) => {
  ...
  const onConfirm = async (id: number): Promise<void> => {
    setLoading(true)
    await deleteApi(`${baseUrl}/api/locations/${id}`)
    setOpen(false)
    setLoading(false)
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align='end'>
          ...
          <DropdownMenuItem
            onClick={() => {
              setLoading(true)
              router.push(`/dashboard/admin/location/${data.id}`)}}
          >
